...
pub unsafe extern "C" fn metaquick_summon_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    start_meta_quick(fighter, 10 * 60);
...
unsafe fn handle_start_metaquick(fighter: &mut L2CFighterCommon) {
    ...
    if fighter.is_situation(*SITUATION_KIND_AIR) {
        ...
        {
            fighter.change_status(statuses::metaknight::METAQUICK_SUMMON.into(), false.into());
...
pub unsafe fn run(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
    ...
    MeterModule::set_meter_per_level(fighter.battle_object, 60.0);
    MeterModule::set_meter_cap(fighter.battle_object, 1);
    ...
    if MeterModule::level(fighter.object()) >= 1 {
...
unsafe extern "C" fn update_meta_quick_timer(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
...
unsafe extern "C" fn check_apply_speeds(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
...
unsafe extern "C" fn check_reset(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
...
pub unsafe extern "C" fn is_meta_quick(fighter: &mut smash::lua2cpp::L2CFighterCommon) -> bool {
    let timer = VarModule::get_int(fighter.object(), vars::common::instance::GIMMICK_TIMER);
    if timer > 0 {
        return true;
    }
...
pub unsafe extern "C" fn start_meta_quick(fighter: &mut smash::lua2cpp::L2CFighterCommon, length: i32) {
...
unsafe extern "C" fn kill_quick_effect(fighter: &mut smash::lua2cpp::L2CFighterCommon) {
...
unsafe extern "C" fn apply_status_speed_mul(fighter: &mut smash::lua2cpp::L2CFighterCommon, mul: f32) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, statuses::metaknight::METAQUICK_SUMMON, metaquick_summon_pre);
    agent.status(Main, statuses::metaknight::METAQUICK_SUMMON, metaquick_summon_main);
    agent.status(End, statuses::metaknight::METAQUICK_SUMMON, metaquick_summon_end);
}