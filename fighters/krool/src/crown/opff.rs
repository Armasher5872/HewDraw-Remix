...
unsafe fn krown_reflect_fix(weapon: &mut smash::lua2cpp::L2CFighterBase) {
    ...
    // update facing direction depending on whether the krown is traveling forwards or backwards
    let lr = PostureModule::lr(boma);
    let speed_x = KineticModule::get_sum_speed_x(boma, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_ALL);
    if weapon.is_status(*WEAPON_KROOL_CROWN_STATUS_KIND_THROW) && weapon.motion_frame() <= 40.0 { // F40 is the frame that krown reverses direction
        PostureModule::reverse_lr(boma);
        PostureModule::update_rot_y_lr(boma);
    }

    // restart crown status
