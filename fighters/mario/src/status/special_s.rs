...
unsafe extern "C" fn special_s_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.is_situation(*SITUATION_KIND_GROUND) {
        ...
        VarModule::on_flag(fighter.battle_object, vars::mario::status::SPECIAL_S_GROUND_START);
        ...
        VarModule::off_flag(fighter.battle_object, vars::mario::status::SPECIAL_S_GROUND_START);
        MotionModule::change_motion(fighter.module_accessor, Hash40::new("special_air_s"), 0.0, 1.0, false, 0.0, false, false);
    }
    special_s_set_kinetic(fighter, false);
...
unsafe extern "C" fn special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if StatusModule::is_situation_changed(fighter.module_accessor) {
        if fighter.is_situation(*SITUATION_KIND_GROUND) {
            ...
            KineticModule::change_kinetic(fighter.module_accessor, *FIGHTER_KINETIC_TYPE_FALL);
            GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
            special_s_set_kinetic(fighter, true);
    ...
    if fighter.status_frame() == 30 {
        if fighter.is_situation(*SITUATION_KIND_AIR) {
            ...
            let air_speed_x_mul = ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_end_speed_x_stable_mul");
            let air_control_x_mul = ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_end_control_x_mul");
            ...
            let air_accel_y_mul = ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_end_accel_y_mul");
...
unsafe extern "C" fn special_s_set_kinetic(fighter: &mut L2CFighterCommon, edge_slide: bool) {
    if !VarModule::is_flag(fighter.battle_object, vars::mario::instance::SPECIAL_S_DISABLE_STALL) {
        ...
        let start_speed_y = if edge_slide {
            if fighter.status_frame() < 25 {
                ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.edge_early_start_speed_y")
            } else {
                ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.edge_late_start_speed_y")
            }
        } else {
            ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_start_speed_y")
        };
        let air_accel_y_mul = ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_start_accel_y_mul");
    ...
    let speed_x_mul = if fighter.is_situation(*SITUATION_KIND_GROUND) {
        ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.ground_start_speed_x_mul")
    } else {
        ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_start_speed_x_mul")
    };
    let accel_x_mul = if fighter.is_situation(*SITUATION_KIND_GROUND) {
        ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.ground_start_accel_x_mul")
    } else {
        ParamModule::get_float(fighter.battle_object, ParamType::Agent, "param_special_s.air_start_accel_x_mul")
    };
...
unsafe extern "C" fn status_stub(fighter: &mut L2CFighterCommon) -> L2CValue {
    return false.into();
}

pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_S, special_s_pre);
    agent.status(Init, *FIGHTER_STATUS_KIND_SPECIAL_S, status_stub);
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_S, special_s_main);
    agent.status(Exec, *FIGHTER_STATUS_KIND_SPECIAL_S, status_stub);
    agent.status(Exit, *FIGHTER_STATUS_KIND_SPECIAL_S, status_stub);
    agent.status(End, *FIGHTER_STATUS_KIND_SPECIAL_S, status_stub);
}