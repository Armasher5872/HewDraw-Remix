...
unsafe extern "C" fn special_lw_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    VarModule::on_flag(fighter.battle_object, vars::common::instance::CHECK_HOLD_INPUT_INSTANCE);
...
unsafe extern "C" fn special_lw_hit_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.battle_object, vars::common::instance::CHECK_HOLD_INPUT_INSTANCE) {
        VarModule::off_flag(fighter.battle_object, vars::common::instance::CHECK_HOLD_INPUT_INSTANCE);
        VarModule::on_flag(fighter.battle_object, vars::edge::status::SPECIAL_LW_HOLD);
    }

    return smashline::original_status(Main, fighter, *FIGHTER_EDGE_STATUS_KIND_SPECIAL_LW_HIT)(fighter);
}

pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_LW, special_lw_main);
    agent.status(Main, *FIGHTER_EDGE_STATUS_KIND_SPECIAL_LW_HIT, special_lw_hit_main);
}