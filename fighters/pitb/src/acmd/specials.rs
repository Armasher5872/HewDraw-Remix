...
unsafe extern "C" fn game_specialhistart(agent: &mut L2CAgentBase) {
    ...
    let boma = agent.boma();
    frame(lua_state, 1.0);
    FT_MOTION_RATE_RANGE(agent, 1.0, 15.0, 21.0);
    if is_excute(agent) {
