...
unsafe fn status_Catch_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    // grab clanks are universally enabled on F4 of the grab status
    let frame = fighter.global_table[CURRENT_FRAME].get_i32();
    if frame == 3 {
        GrabModule::set_rebound(fighter.module_accessor, true);
    }
    // and are disabled when the grab ends (but not later than F16)
    if GrabModule::is_rebound(fighter.module_accessor) 
    && (frame >= 15 || fighter.is_flag(*FIGHTER_STATUS_CATCH_FLAG_CATCH_WAIT)) {
