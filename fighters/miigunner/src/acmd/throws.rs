...
unsafe extern "C" fn game_catch(agent: &mut L2CAgentBase) {
	...
	if is_excute(agent) {
		CATCH(agent, 0, Hash40::new("top"), 4.5, 0.0, 6.6, 0.0, Some(0.0), Some(6.6), Some(12.0), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
	}
...
unsafe extern "C" fn game_catchdash(agent: &mut L2CAgentBase) {
	...
	frame(lua_state, 9.0);
	if is_excute(agent) {
		CATCH(agent, 0, Hash40::new("top"), 4.5, 0.0, 6.7, 0.0, Some(0.0), Some(6.7), Some(12.4), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
	}
	game_CaptureCutCommon(agent);
	wait(lua_state, 2.0);
	if is_excute(agent) {
