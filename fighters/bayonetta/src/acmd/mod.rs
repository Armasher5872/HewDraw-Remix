...
pub unsafe fn CHECK_BA(agent: &mut L2CAgentBase, enable: bool) -> bool {
    ...
    if agent.is_button_on(Buttons::Special | Buttons::Attack)
    && (InputModule::get_trigger_count(agent.battle_object, Buttons::Special) >= buffer // check if held longer than buffer
