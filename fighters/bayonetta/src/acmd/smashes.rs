...
unsafe extern "C" fn game_attacklw4(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        agent.on_flag(*FIGHTER_STATUS_ATTACK_FLAG_START_SMASH_HOLD);
        if agent.is_flag(*FIGHTER_STATUS_ATTACK_FLAG_SMASH_SMASH_HOLD_TO_ATTACK) {
            ArticleModule::generate_article(boma, *FIGHTER_BAYONETTA_GENERATE_ARTICLE_WICKEDWEAVELEG, false, -1); // 21 -> f22 ingame
        }
    }
    ...
    FT_MOTION_RATE_RANGE(agent, 28.0, 69.0, 38.0); // 65 -> 63
    if is_excute(agent) {
        CHECK_BA(agent, true);
