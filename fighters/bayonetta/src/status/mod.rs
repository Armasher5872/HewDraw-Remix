...
unsafe extern "C" fn jump_refresh(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && frame <= fighter.get_param_float("param_special_hi", "jump_count_reset_frame") {
        ...
        PostureModule::set_pos(fighter.module_accessor, &Vector3f{x: PostureModule::pos_x(fighter.module_accessor), y: PostureModule::pos_y(fighter.module_accessor) - (frame * 1.0), z: PostureModule::pos_z(fighter.module_accessor)});
    }
    0.into()
}

unsafe extern "C" fn should_use_special_s_callback(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.is_flag(*FIGHTER_BAYONETTA_INSTANCE_WORK_ID_FLAG_DISABLE_AIR_SPECIAL_S)
    || (fighter.get_int(*FIGHTER_BAYONETTA_INSTANCE_WORK_ID_INT_SPECIAL_AIR_S_REUSE_FRAME) <= 0
