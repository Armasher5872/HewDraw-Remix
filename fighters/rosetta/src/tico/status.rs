...
unsafe extern "C" fn standby_main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    KineticModule::unable_energy_all(weapon.module_accessor);
    MotionModule::change_motion(weapon.module_accessor, Hash40::new("free_standby"), 0.0, (27.0 / 21.0), false, 0.0, false, false);//21st frame starts pop status
...
unsafe extern "C" fn standby_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    {//if rosaliner is not holding special or if she gets interrupted, do something
...
unsafe extern "C" fn pop_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let frame = MotionModule::frame(weapon.module_accessor).min(12.0);
    let scale = 1.1 + ((0.4 / 12.0) * frame);
